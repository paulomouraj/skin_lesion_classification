...
############################################################################################################
## Pipeline to train a ResNet model on the skin lesion dataset, created to be run in a Python script.     ##
## The model is trained on the training set and the best model is saved.                                  ##
## The training metrics are saved to a file.                                                             ##
############################################################################################################
...
# Load the dataset
train_data = CustomDataset(train_dir, metadata_train,
                           train=True, num_channels=3, transform=img_transform)

# separate into train and validation
train_size = int(0.8 * len(train_data))
val_size = len(train_data) - train_size
train_dataset, val_dataset = torch.utils.data.random_split(
    train_data, [train_size, val_size], generator=torch.Generator().manual_seed(0))
...
train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True,
                          generator=torch.Generator().manual_seed(0), num_workers=os.cpu_count())
val_loader = DataLoader(val_dataset, batch_size=batch_size,
                        shuffle=False, num_workers=os.cpu_count())

# train model
model.to(device)
class_weights = [0.7005531, 0.24592265, 0.95261733,
                 3.64804147, 1.20674543, 13.19375, 12.56547619, 5.04219745]
...
for epoch in range(num_epochs):
    ...
    with tqdm(train_loader, desc=f'Train Epoch {epoch + 1}/{num_epochs}', unit='batch') as train_tqdm:
        for images, labels in train_tqdm:
            ...
            train_tqdm.set_postfix({'train_loss': train_loss.item(
...
with open(file_path, "w") as file:
    ...
    for epoch in range(num_epochs):
        file.write(
            f"{epoch+1}\t{train_losses[epoch]}\t{val_losses[epoch]}\t{train_accuracies[epoch]}\t{val_accuracies[epoch]}\n")
